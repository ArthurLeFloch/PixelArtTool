...
void draw_spectrum (SDL_Renderer *renderer, SDL_Rect *output, HSV_Color selected) {
	...
	SDL_Rect rect = { output->x + roundf (output->w * selected.h) - 10,
					  output->y + roundf (200 * (1 - selected.s)) - 10, 20, 20 };
...
void render_status_bar (SDL_Renderer *renderer, TTF_Font *font, PixelArt *pixel_art, SDL_Point pos,
						SDL_Color bg_color) {
